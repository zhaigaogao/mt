(function (factory) {
    ...
    function format (str, fmat) {
        if (!str) return false;
        str = str.split('/');
        fmat = fmat || 'yyyy-MM-dd';

        var paramDate = new Date(str[0], parseInt(str[1]) - 1, str[2]);
        ...
        if (/(y+)/.test(result)) result = result.replace(RegExp.$1, (paramDate.getFullYear() + '').substr(4 - RegExp.$1.length));
        for (var k in o) {
    ...
    Calendar.prototype.init = function () {
        ...
        this.createHeader(function () {
            ...
            // 显示/隐藏 月/年 份选择
            var i = 0, x = 0;
            for (i = 0 ; i < calenTitles.length ; i++) {
    ...
    Calendar.prototype.bind = function (elements) {
        var $this = this;
        var aCalendars = elements;
    ...
    Calendar.prototype.createCalenList = function (data, setTitle) {
        ...
        var i = 0, oSpan, oNum, n, oDate;

        for (i = lastDay ; i > 0 ; i--) lastMonths.push(i);
        ...
        for (i = 0 ; i < lastMonthDay ; i++) {
            ...
            if (lastMonths[i] == tDay && data.m == 1 && !data.y && !data.d
                || !data.y && Number($this.fixDate.m) + 1 == tMonth && $this.fixDate.d == lastMonths[i]) {
        ...
        for (i = 0 ; i < dSun ; i++) {
            ...
            // 设置是否小于用户定义的开始日期
            if (tYear <= $this.fixDate.y && tMonth <= $this.fixDate.m && n < data.d
                || tYear <= $this.fixDate.y && tMonth < $this.fixDate.m) {
        ...
        for (i = 0 ; i < nextMonths ; i++) {
            ...
            if (n == tDay && data.m == -1 && !data.y && !data.d
                || !data.y && $this.fixDate.m - 1 == tMonth && $this.fixDate.d == n) {
                toolClass(oNum, 'today');
            }
    ...
    Calendar.prototype.createTime = function (obj, date, today, past) {
        ...
        for (i = 0 ; i < child.length ; i++) {
            ...
            if ($this.hoursPast && ((mNow < 0 && yNow <= 0) || (today == day &&  child[i].time <= hours)
                || (mNow <= 0 && yNow <= 0 && today < day))) {
